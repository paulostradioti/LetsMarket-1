...
namespace LetsMarket
{
    ...
    public class Repository
    {
        ...
        public static void InitializeDatabase()
        {
            ...
            {
                Funcionarios.Add(new Employee { Nome = "Admin", Login = "admin", Password = "admin" });
                Save(DatabaseOption.Funcionarios);
            }

            if (!File.Exists(_productsDb) && File.Exists("Database/dados.csv"))
            {
                var faker = new Bogus.DataSets.Commerce();

                using (var reader = new StreamReader("Database/dados.csv"))
